...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const handleAdd = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // منتج فيه خيارات (روائح/ألوان) - نوجّه لصفحة المنتج لاختيار الخيار أولاً
    if (hasVariants) {
    ...
    <div className="card-surface group flex flex-col h-full overflow-hidden">
      ...
      <Link href={`/products/${product.id}`} className="p-3.5 flex flex-col gap-1.5 flex-1">
        ...
        <div className="mt-auto flex items-baseline gap-2">
          <span className="font-bold text-brand-purple">{product.price.toLocaleString("en-US")} دج</span>
          {product.originalPrice && product.originalPrice > product.price && (
